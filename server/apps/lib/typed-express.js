...
export type AuthRequest<User, Params: { [string]: string } = {}> = Request<
  Params
> & {
  +user: User
};
...
export type Middleware<ReqIn, ReqOut> = ReqIn => [
  ReqOut,
  (ReqIn, Response, Next) => void
];
...
class Chain<Req: Request<>> {
  ...
  use<ReqOut: Request<>>(middleware: Middleware<Req, ReqOut>): Chain<ReqOut> {
    ...
    return new Chain(this.router, this.action, this.path, [
      ...this.middleware,
      pair || middleware
    ]);
  }
  ...
  handle(handler: Handler<Req>) {
    (this.router: any)[this.action].apply(this.router, [
      this.path,
      ...this.middleware,
      handler
    ]);
