...
module.exports = async (db, verbose) => {
  if (verbose) {
    console.log('Seeding Open Charlotte');
  }
  ...
  await Promise.all(
    cmpdStations.map(async station => {
      if (verbose) {
      ...
      await db.affiliation.create({
        ...
        address_city: station.city,
        address_state: station.state,
        address_zip: station.zip,
        phone: station.phone
      });
    })
  );
  ...
  await Promise.all(
    cmsSchools.map(async school => {
      ...
      await db.affiliation.create({
        ...
        address_state: school.state,
        address_zip: school.zip
      });
    })
  );
};
